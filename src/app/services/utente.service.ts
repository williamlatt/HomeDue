...
export class UtenteService {
    ...
    login(account: Account): Observable<Utente> {
        return this.http.post<Utente>(URL.LOGIN, account, {observe: 'response'}).pipe(
            map((resp: HttpResponse<Utente>) => {
                ...
                this.storage.set(GRUPPO_STORAGE, 0);
    ...
    register(utente: Utente) {
        return this.http.post<Utente>(URL.REGISTER_UTENTE, utente);
    }
    logout() {
        this.authToken = null;
        this.loggedIn$.next(true);
    ...
    signup(newu: newU) {
        return this.http.post<newU>(URL.REGISTER_UTENTE, newu);
